...
- When a fact finally enters memory, mark the abstention question that asked for it **[retired]**
  in its `expected:` line and append a new abstention question in its place. It stays in the file
  and in the log; the report just stops counting it. Skip this and the category empties out in
  silence — the questions keep passing while nothing tests for made-up answers any more.
- Distribution at full size: single-fact 6 · synthesis 3 · temporal 3 · update 3 · abstention 3 ·
  confusion-trap 2+ · protocol 3
...
Run them with `python3 memory/tools/eval.py run` (interactive), or
`eval.py record <Qn> <category> <language> <0|1> <turns> <tokens> "<note>"` when an assistant
produced the answer and you scored it. Results land in `eval-log.csv`.

## Question format

```
## Q<n>
category: single-fact | synthesis | temporal | update | abstention | confusion-trap | protocol
...
| protocol | Whether a rule gate actually fires | "I ask for X. What is your first step?" |
